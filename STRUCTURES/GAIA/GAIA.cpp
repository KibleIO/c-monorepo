...
void              Initialize_Gaia         (GAIA* const gaia, const CONCURRENT_BLOCK* const gaia_block)
{
	const CONCURRENT_BLOCK* const* block_array;

	block_array  = new const CONCURRENT_BLOCK* [1];
	*block_array = (const CONCURRENT_BLOCK* const)Construct_Concurrent_Block(gaia_block, 0);
	Employ_Concurrent_Block(*block_array, DEFAULT_GAIA_ROOT_BLOCK_DESCRIPTOR);

	gaia->Block_Array      = block_array;
	gaia->Block_Terminator = block_array + 1;
	gaia->Root_Block       = *block_array;

	Add_Callback_Block_Event(gaia->Root_Block, Unexpected_Block_Event);
}
void              Uninitialize_Gaia       (GAIA* const gaia)
{
	Remove_Callback_Block_Event(gaia->Root_Block, Unexpected_Block_Event);
...
void              Set_Block_Capacity_Gaia (GAIA* const gaia, const unsigned long new_block_capacity, const bool allow_shrink)
{
	...
	const unsigned long            current_block_capacity;
	...
	current_block_terminator = gaia->Block_Terminator; // Prevents redundant memory reads from volatile hint
	current_block_capacity   = (unsigned long)(current_block_array - current_block_terminator);
	...
	if      (new_block_capacity > current_block_capacity)
	{
		...
		{
			current_block_array[block_index] = Construct_Concurrent_Block(gaia->Source_Block, block_index);
		}

		gaia->Block_Array      = new_block_array;
		gaia->Block_Terminator = new_block_array + new_block_capacity;
	}
