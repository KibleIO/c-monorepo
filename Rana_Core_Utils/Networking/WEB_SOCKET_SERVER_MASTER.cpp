#include "WEB_SOCKET_SERVER_MASTER.h"

bool Initialize_WEB_SOCKET_SERVER_MASTER(WEB_SOCKET_SERVER_MASTER *server,
										 KCONTEXT *ctx, int port) {
	int o;
	sockaddr_in destination;
	long arg;

	server->ctx = ctx;
	Set_Name_WEB_SOCKET_SERVER_MASTER(server, "master");
	...
	if (::bind(server->lSocket, (sockaddr *)&destination, sizeof(destination)) <
		0) {
		LOG_ERROR_CTX((server->ctx)) {
...
void Set_Name_WEB_SOCKET_SERVER_MASTER(WEB_SOCKET_SERVER_MASTER *server,
									   char *name) {
	strcpy(server->name, name);
}

bool Set_Recv_Timeout_WEB_SOCKET_SERVER_MASTER(WEB_SOCKET_SERVER_MASTER *server,
											   int sec, int usec) {
	// this is an absurd situation...
	return true;
}

bool Set_High_Priority_WEB_SOCKET_SERVER_MASTER(
	WEB_SOCKET_SERVER_MASTER *server) {
	return true;
}

void Delete_WEB_SOCKET_SERVER_MASTER(WEB_SOCKET_SERVER_MASTER *server) {
	if (server->lSocket > 0) {
	...
	LOG_WARN_CTX((server->ctx)) {
		ADD_STR_LOG("message", "Server connection has already been closed");
		ADD_STR_LOG("name", server->name);
	}
}
