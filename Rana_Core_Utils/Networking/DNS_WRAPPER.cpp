...
void callback(void* arg, int32_t status, int32_t timeouts, hostent* host) {
    ...
    if(!host || status != ARES_SUCCESS) {
		log_err(string("Failed to lookup ") + string(ares_strerror(status)));
...
uint32_t* ip_addr, const char* dns_address, uint32_t timeout_val) {
    ...
    if (status != ARES_SUCCESS) {
		log_err(string("ares_library_init: ") + string(ares_strerror(status)));
        return false;
    }
    ...
    if (status != ARES_SUCCESS) {
		log_err(string("ares_init_options: ") + string(ares_strerror(status)));
        return false;
    }
	...
	if (nfds == 0) {
		log_err(string("ares_fds: ") + string(ares_strerror(status)));
		goto cleanup;
	}
