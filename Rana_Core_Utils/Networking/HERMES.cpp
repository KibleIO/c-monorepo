...
string Connection_Name(int n) {
	...
	case 5:
		return "data";
	default:
		log_err(((const JSON_TYPE){{"message", "no name for connection"},
								   JSON_TYPE_END}));
		return "ERROR";
	}
}
...
void Connect_HERMES_SERVER(HERMES_SERVER* hs, int port, int baseport) {
	...
	if (hs->enc_eng) {
		...
		ENCRYPTION_PROFILE* enc_prof =
			hs->enc_eng
				->profiles_available[hs->enc_eng->number_of_profiles - 1];
	...
	while (hs->connected) {
		...
		if (flag == HERMES_STATUS) {
			...
			if (port > max_port) {
				log_err(((const JSON_TYPE){{"message", "out of ports"},
										   JSON_TYPE_END}));
				delete server;
				hs->err = EPIPE;
				break;
			...
			if (hs->enc_eng) {
				...
				ENCRYPTION_PROFILE* enc_prof =
					hs->enc_eng
						->profiles_available[hs->enc_eng->number_of_profiles -
											 1];
...
bool Create_CLIENT_CONNECTION(HERMES_CLIENT* hc, uint8_t type) {
	...
	if (hc->enc_eng) {
		if (!Add_Profile_ENCRYPTION_ENGINE(hc->enc_eng, Connection_Name(type),
										   hc->enc_eng->active_profile)) {
			log_err(((const JSON_TYPE){
				{"message", "could not add encryption profile"},
				JSON_TYPE_END}));
		...
		ENCRYPTION_PROFILE* enc_prof =
			hc->enc_eng
				->profiles_available[hc->enc_eng->number_of_profiles - 1];
...
bool Connect_HERMES_CLIENT(HERMES_CLIENT* hc, string ip, int port, int* types) {
	...
	if (hc->enc_eng) {
		if (!Add_Profile_ENCRYPTION_ENGINE(hc->enc_eng, "hermes_init",
										   hc->enc_eng->active_profile)) {
			log_err(((const JSON_TYPE){
				{"message", "could not add encryption profile hermes_init"},
				JSON_TYPE_END}));
