...
GRAPHICS* graphics, uint8_t* buffer, SCREEN_DIM screen_dim) {
	graphics->Buffer      = buffer;
	graphics->screen_dim  = screen_dim;
	graphics->Width_Clip  = screen_dim.sw;
	graphics->Height_Clip = screen_dim.h;
	graphics->X_clip      = 0;
	graphics->Y_clip      = 0;
	graphics->ContourX    = new long[screen_dim.h * 2];
...
void ClearScreen_GRAPHICS(GRAPHICS* graphics, int color) {
	...
	for (int y = graphics->screen_dim.h - 1; y >= 0; y--) {
		for (int x = graphics->screen_dim.sw - 1; x >= 0; x--) {
			buff[y * graphics->screen_dim.sw + x] = color;
		}
	}
}
...
void DrawPoint_Transparent_GRAPHICS(
GRAPHICS* graphics, int x, int y, int col1) {
	if (x < graphics->Width_Clip + graphics->X_clip &&
	x >= graphics->X_clip &&
	y < graphics->Height_Clip + graphics->Y_clip &&
	y >= graphics->Y_clip) {
		...
		unsigned char* bg =
		(unsigned char*)&((int*)graphics->Buffer)[
		x + (y * graphics->screen_dim.bw)];
		...
		((int*)graphics->Buffer)[x + (y * graphics->screen_dim.bw)] = result;
	}
}

void DrawPoint_Transparent_GRAPHICS_UNSAFE(
GRAPHICS* graphics, int x, int y, int col1) {
	if (((unsigned char*)&col1)[3] == 255) {
...
GRAPHICS* graphics, int x1, int y1, int x2, int y2, int color) {
	if (graphics->Transparent) {
...
void DrawLine_Transparent_GRAPHICS(
GRAPHICS* graphics, int x1, int y1, int x2, int y2, int color) {
	int dx, dy, inx, iny, e;
	...
	while (cnt--) {
		if ((y >= 0) && (y < graphics->screen_dim.bw)) {
			if (x < graphics->ContourX[y * graphics->screen_dim.h + 0]) {
				graphics->ContourX[y * graphics->screen_dim.h + 0] = x;
			}
			if (x > graphics->ContourX[y * graphics->screen_dim.h + 1]) {
...
void FillTriangle_GRAPHICS(
GRAPHICS* graphics, int x0, int y0, int x1, int y1, int x2, int y2, int color) {
	if (graphics->Transparent) {
		FillTriangle_Transparent_GRAPHICS(
		graphics, x0, y0, x1, y1, x2, y2, color);
	} else {
		FillTriangle_Opaque_GRAPHICS(graphics, x0, y0, x1, y1, x2, y2, color);
	}
}

void FillTriangle_Transparent_GRAPHICS(
GRAPHICS* graphics, int x0, int y0, int x1, int y1, int x2, int y2, int color) {
	long y;
...
void FillTriangle_Opaque_GRAPHICS(
GRAPHICS* graphics, int x0, int y0, int x1, int y1, int x2, int y2, int color) {
	long y;
...
void DrawLine_Height_GRAPHICS(
GRAPHICS* graphics, int x, int y, int height, int color) {
	if (graphics->Transparent) {
...
void DrawLine_Height_GRAPHICS_UNSAFE(
GRAPHICS* graphics, int x, int y, int height, int color) {
	if (graphics->Transparent) {
		DrawLine_Height_Transparent_GRAPHICS_UNSAFE(
		graphics, x, y, height, color);
	} else {
		DrawLine_Height_Opaque_GRAPHICS_UNSAFE(graphics, x, y, height, color);
	}
}

void DrawLine_Height_Transparent_GRAPHICS(
GRAPHICS* graphics, int x, int y, int height, int color) {
	if (x < graphics->X_clip || x > graphics->Width_Clip + graphics->X_clip) {
		return;
	}
	...
	if (height + y > graphics->Height_Clip + graphics->Y_clip) {
		height = (graphics->Height_Clip + graphics->Y_clip) - y;
	}
...
void DrawLine_Height_Transparent_GRAPHICS_UNSAFE(
GRAPHICS* graphics, int x, int y, int height, int color) {
	for (int i = 0; i < height; i++) {
		DrawPoint_Transparent_GRAPHICS_UNSAFE(graphics, x, y + i, color);
	}
...
void DrawLine_Height_Opaque_GRAPHICS(
GRAPHICS* graphics, int x, int y, int height, int color) {
	if (x < graphics->X_clip || x > graphics->Width_Clip + graphics->X_clip) {
		return;
	}
...
void DrawLine_Height_Opaque_GRAPHICS_UNSAFE(
GRAPHICS* graphics, int x, int y, int height, int color) {
	for (int i = 0; i < height; i++) {
		DrawPoint_Opaque_GRAPHICS_UNSAFE(graphics, x, y + i, color);
	}
...
void DrawLine_Width_GRAPHICS_UNSAFE(
GRAPHICS* graphics, int x, int y, int width, int color) {
	if (graphics->Transparent) {
		DrawLine_Width_Transparent_GRAPHICS_UNSAFE(
		graphics, x, y, width, color);
	} else {
...
void DrawLine_Width_Transparent_GRAPHICS_UNSAFE(
GRAPHICS* graphics, int x, int y, int width, int color) {
	for (int i = 0; i < width; i++) {
		DrawPoint_Transparent_GRAPHICS_UNSAFE(graphics, x + i, y, color);
	}
...
void DrawLine_Width_Opaque_GRAPHICS(
GRAPHICS* graphics, int x, int y, int width, int color) {
	if (y < graphics->Y_clip || y > graphics->Height_Clip + graphics->Y_clip) {
		return;
	}
...
void DrawLine_Width_Opaque_GRAPHICS_UNSAFE(
GRAPHICS* graphics, int x, int y, int width, int color) {
	for (int i = 0; i < width; i++) {
		DrawPoint_Opaque_GRAPHICS_UNSAFE(graphics, x + i, y, color);
	}
...
void DrawSquare_GRAPHICS(
GRAPHICS* graphics, int x, int y, int w, int h, int color) {
	if (graphics->Transparent) {
...
void DrawSquare_Transparent_GRAPHICS(
GRAPHICS* graphics, int x, int y, int w, int h, int color) {
...
void DrawSquare_Opaque_GRAPHICS(
GRAPHICS* graphics, int x, int y, int w, int h, int color) {
	if (x > graphics->Width_Clip + graphics->X_clip) return;
	if (y > graphics->Height_Clip + graphics->Y_clip) return;
	if (x < graphics->X_clip) {
...
void FillSquare_Transparent_GRAPHICS(
GRAPHICS* graphics, int x, int y, int w, int h, int color) {
	if (x > graphics->Width_Clip + graphics->X_clip) {
		return;
	}
	...
	for (int i = 0; i < h; i++) {
		DrawLine_Width_Transparent_GRAPHICS_UNSAFE(
		graphics, x, y + i, w, color);
	}
}

void FillSquare_Opaque_GRAPHICS(
GRAPHICS* graphics, int x, int y, int w, int h, int color) {
	if (x > graphics->Width_Clip + graphics->X_clip) return;
	if (y > graphics->Height_Clip + graphics->Y_clip) return;
	if (x < graphics->X_clip) {
...
void DrawCircle_GRAPHICS(
GRAPHICS* graphics, int x0, int y0, int radius, int color) {
	if (graphics->Transparent) {
...
void DrawCircle_Transparent_GRAPHICS(
GRAPHICS* graphics, int x0, int y0, int radius, int c) {
	int x = radius, y = 0;
	int radiusError = 1 - x;
...
void DrawCircle_Opaque_GRAPHICS(
GRAPHICS* graphics, int x0, int y0, int radius, int c) {
	int x = radius, y = 0;
	int radiusError = 1 - x;
...
void FillCircle_Transparent_GRAPHICS(
GRAPHICS* graphics, int x0, int y0, int radius, int c) {
	int x = radius, y = 0;
	int radiusError = 1 - x;

	while(x >= y) {
		DrawLine_Width_Transparent_GRAPHICS(
		graphics, -x + x0, y + y0, x * 2, c);
		DrawLine_Width_Transparent_GRAPHICS(
		graphics, -y + x0, x + y0, y * 2, c);
		DrawLine_Width_Transparent_GRAPHICS(
		graphics, -x + x0, -y + y0, x * 2, c);
		DrawLine_Width_Transparent_GRAPHICS(
		graphics, -y + x0, -x + y0, y * 2, c);
...
void FillCircle_Opaque_GRAPHICS(
GRAPHICS* graphics, int x0, int y0, int radius, int c) {
	int x = radius, y = 0;
	int radiusError = 1 - x;
...
void FillRoundedRect_GRAPHICS(
GRAPHICS* graphics, int x, int y, int w, int h, int r, Color color) {
	...
	if (r <= 0) {
		...
		// Clamp the rounding constant so that it cannot be
		//bigger than half the smallest dimension
		...
		FillSquare_GRAPHICS(
		graphics, x + r + innerWidth, y + r, r, innerHeight + 1, color);

		// Fill arcs at the corners to created rounded edges
		FillQuarterArc_GRAPHICS(
		graphics, x + innerWidth + r, y, r, r, FIRST_QUADRANT, color);
		FillQuarterArc_GRAPHICS(graphics, x, y, r, r, SECOND_QUADRANT, color);
		FillQuarterArc_GRAPHICS(
		graphics, x, y + innerHeight + r, r, r, THIRD_QUADRANT, color);
		FillQuarterArc_GRAPHICS(
		graphics, x + innerWidth + r, y + innerHeight + r,
		r, r, FOURTH_QUADRANT, color);
	}
}

void FillQuarterArc_GRAPHICS(
GRAPHICS* graphics, int x, int y, int w, int h, int quadrant, Color color) {
	// Prevent function from continuing if dimensions are invalid
	if (x < 0 || y < 0 || w < 2 || h < 2) {
		log_err("invalid arc: " + to_string(x) + " " + to_string(y) +
	...
	quadrant != THIRD_QUADRANT && quadrant != FOURTH_QUADRANT) {
		log_err("invalid quadrant: " + to_string(quadrant));
		return;
	}
	...
	// Each is positive or negative one,
	//depending on the quadrant the arc is being filled in
...
// Drawing a thick "line" really involves drawing a
//rectangle where thickness is the height
void DrawThickHorizontalLine_GRAPHICS(
GRAPHICS* graphics, int x, int y, int w, int thickness, Color color) {
...
void DrawThickVerticalLine_GRAPHICS(
GRAPHICS* graphics, int x, int y, int h, int thickness, Color color) {
	FillSquare_GRAPHICS(graphics, x, y, thickness, h, color);
}

// Draw a rect with the desired thickness by drawing four lines
void DrawThickRect_GRAPHICS(
GRAPHICS* graphics, int x, int y, int w, int h, int thickness, Color color) {
	DrawThickHorizontalLine_GRAPHICS(graphics, x, y, w, thickness, color);
	DrawThickHorizontalLine_GRAPHICS(
	graphics, x, y + h - thickness, w, thickness, color);
	DrawThickVerticalLine_GRAPHICS(graphics, x, y, h, thickness, color);
	DrawThickVerticalLine_GRAPHICS(
	graphics, x + w - thickness, y, h, thickness, color);
}

void DrawThickRoundedRect_GRAPHICS(
GRAPHICS* graphics, int x, int y, int w, int h,
int thickness, int r, Color color) {
	// Check to see that the rect trying to be drawn is valid
	if (x < 0 || y < 0 || w < 0 || h < 0) {
		log_err("invalid rect: " + to_string(x) + " " + to_string(y) +
		" " + to_string(y) + " " + to_string(h));
		return;
	}
    ...
    int halfMinDimension = h < w ? h >> 1 : w >> 1;
		...
		// Clamp the rounding constant so that it cannot be bigger
		//than half the smallest dimension
		if(r > halfMinDimension) {
			r = halfMinDimension;
		}
		// Internal width of the rectangular drawing between the quarter circles
		int innerWidth = w - (r << 1);
		// Internal height of the rectangular drawing
		//between the quarter circles
		int innerHeight = h - (r << 1);

		// Draw thick horizontal/vertical lines
		DrawThickHorizontalLine_GRAPHICS(
		graphics, x + r, y, w - (r << 1), thickness, color);
		DrawThickHorizontalLine_GRAPHICS(
		graphics, x + r, y + h - thickness, w - (r << 1), thickness, color);
		DrawThickVerticalLine_GRAPHICS(
		graphics, x, y + r, h - (r << 1), thickness, color);
		DrawThickVerticalLine_GRAPHICS(
		graphics, x + w - thickness, y + r, h - (r << 1), thickness, color);

    	DrawThickQuarterArc_GRAPHICS(
		graphics, x + innerWidth + r, y, r, r, thickness,
		FIRST_QUADRANT, color);
    	DrawThickQuarterArc_GRAPHICS(
		graphics, x, y, r, r, thickness, SECOND_QUADRANT, color);
    	DrawThickQuarterArc_GRAPHICS(
		graphics, x, y + innerHeight + r, r, r, thickness,
		THIRD_QUADRANT, color);
    	DrawThickQuarterArc_GRAPHICS(
		graphics, x + innerWidth + r, y + innerHeight + r, r, r,
		thickness, FOURTH_QUADRANT, color);
	}
}

void DrawQuarterArc_GRAPHICS(
GRAPHICS* graphics, int x, int y, int w, int h, int quadrant, Color color) {
	// Prevent function from continuing if dimensions are invalid
	if (x < 0 || y < 0 || w < 0 || h < 0) {
		log_err("invalid arc: " + to_string(x) + " " + to_string(y) +
...
void DrawThickQuarterArc_GRAPHICS(
GRAPHICS* graphics, int x, int y, int w, int h,
int thickness, int quadrant, Color color) {
